...
def extract_semantic_expressions(ast: Dict) -> Dict[str, List[str]]:
    ...
    for node in ast.get("nodes", []):
        ...
        if not isinstance(label, str):
            continue
        # Remove inline (// ...) and block (/* ... */) comments from the label
        # to prevent semantic anchors from including comment text.
        # Strip Java style single-line comments
        label_no_comments = re.sub(r"//.*", "", label)
        # Strip block comments (possibly spanning multiple tokens)
        label_no_comments = re.sub(r"/\*.*?\*/", "", label_no_comments, flags=re.DOTALL)
        label = label_no_comments
